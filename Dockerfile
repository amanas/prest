...
RUN go mod download
RUN echo
RUN CGO_ENABLED=1 GOOS=linux GOARCH=arm GOARM=7 GO111MODULE=on go build -ldflags "-s -w" -o prestd cmd/prestd/main.go
RUN apt-get update && apt-get install --no-install-recommends -yq netcat=1.10-41.1
